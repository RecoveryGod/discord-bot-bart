# Fixed selling price per unit (€) – auto-synced from WooCommerce API (status=publish only).
# Sale price preferred over regular price. Non-published products excluded.
# Last updated: 2026-07-17
prices = {
    "10010": 20.0,  # SpectraX - GTA Lexis Lua - 1 Month
    "10020": 45.0,  # SpectraX - GTA Lexis Lua - Lifetime
    "10110": 6.99,  # Kernaim - SAND: Raiders of Sophie - 1 Day
    "10120": 19.99,  # Kernaim - SAND: Raiders of Sophie - 1 Week
    "10130": 39.99,  # Kernaim - SAND: Raiders of Sophie - 1 Month
    "10140": 79.99,  # Kernaim - SAND: Raiders of Sophie - 3 Months
    "10150": 199.99,  # Kernaim - SAND: Raiders of Sophie - Lifetime
    "2110": 15.0,  # X-Force – GTA - Essential
    "2130": 25.0,  # X-Force – GTA - Ace 30 days
    "22011": 23.5,  # Cherax – License Key - Standard
    "22012": 46.0,  # Cherax – License Key - Premium
    "22013": 23.5,  # Cherax – License Key - Standard to Premium Upgrade
    "3310": 6.1,  # Midnight - GTA - 30 Days FSL
    "3320": 18.7,  # Midnight - GTA - Lifetime FSL
    "3410": 16.25,  # Fortitude - RDR2 - 30 days
    "3420": 52.4,  # Fortitude - RDR2 - Lifetime
    "3610": 12.0,  # Stand – GTA - Basic
    "3620": 24.5,  # Stand – GTA - Regular
    "3630": 48.0,  # Stand – GTA - Ultimate
    "3640": 12.0,  # Stand – GTA - Basic to Regular Upgrade
    "3650": 24.5,  # Stand – GTA - Regular to Ultimate Upgrade
    "3660": 37.0,  # Stand – GTA - Basic to Ultimate Upgrade
    "3910": 7.3,  # Predator Deadlock - 1 Week
    "3920": 15.6,  # Predator Deadlock - 1 Month
    "3930": 32.0,  # Predator Deadlock - 3 Months
    "3940": 54.0,  # Predator Deadlock - 6 Months
    "3950": 83.0,  # Predator Deadlock - 1 Year
    "4010": 9.0,  # redENGINE FiveM Lua Executor - 1 Week
    "4020": 17.0,  # redENGINE FiveM Lua Executor - 1 Month
    "4030": 40.0,  # redENGINE FiveM Lua Executor - Lifetime
    "4110": 7.0,  # redENGINE FiveM Spoofer - 1 Week
    "4120": 13.2,  # redENGINE FiveM Spoofer - 1 Month
    "4210": 8.87,  # Phaze Mod Menu - 1 Week
    "4220": 17.0,  # Phaze Mod Menu - 1 Month
    "4230": 40.0,  # Phaze Mod Menu - Lifetime
    "4310": 25.8,  # Lexis - GTA - 1 Month
    "4320": 60.2,  # Lexis - GTA - 3 Month
    "4330": 120.0,  # Lexis - GTA - Lifetime
    "4410": 6.1,  # Midnight - CS2 - 30 Days
    "4510": 4.9,  # Midnight - CS 1.6 - 30 Days
    "4610": 9.3,  # Lexis - Apex Legends - 3 Days
    "4620": 14.6,  # Lexis - Apex Legends - 1 Week
    "4630": 24.3,  # Lexis - Apex Legends - 1 Month
    "4640": 44.3,  # Lexis - Apex Legends - 3 Months
    "4650": 140.0,  # Lexis - Apex Legends - Lifetime
    "4710": 12.0,  # 0xCheats – GTA Legacy & Enhance - Enhanced, 30 days
    "4720": 6.0,  # 0xCheats – GTA Legacy & Enhance - Legacy, 7 days
    "4730": 6.0,  # 0xCheats – GTA Legacy & Enhance - Enhanced, 7 days
    "4740": 24.0,  # 0xCheats – GTA Legacy & Enhance - Enhanced, 90 days
    "4750": 12.0,  # 0xCheats – GTA Legacy & Enhance - Legacy, 30 days
    "4760": 24.0,  # 0xCheats – GTA Legacy & Enhance - Legacy, 90 days
    "4810": 2.75,  # Predator Marvel Rivals - 1 Day
    "4820": 7.3,  # Predator Marvel Rivals - 1 Week
    "4830": 15.6,  # Predator Marvel Rivals - 1 Month
    "4840": 33.0,  # Predator Marvel Rivals - 3 Months
    "4850": 54.0,  # Predator Marvel Rivals - 6 Months
    "4860": 83.0,  # Predator Marvel Rivals - 1 Year
    "4910": 23.5,  # Infamous – GTA - Legacy Full
    "4920": 57.5,  # Infamous – GTA - Enhanced Full Plus
    "4930": 70.0,  # Infamous – GTA - Tester Legacy & Enhanced
    "4940": 45.8,  # Infamous – GTA - Legacy Full Plus
    "5010": 41.0,  # Infamous – FiveM - Full
    "5110": 23.5,  # Infamous – RDR2 - Full
    "5210": 13.1,  # Atlas – GTA - Standard Lifetime
    "5220": 17.5,  # Atlas – GTA - VIP 30 Days
    "5230": 5.5,  # Atlas – GTA - VIP 7 Days
    "5240": 35.0,  # Atlas – GTA - VIP 90 Days
    "5250": 52.5,  # Atlas – GTA - VIP Lifetime
    "5310": 4.0,  # MemeSense - CS2 - 14 Days
    "5320": 5.8,  # MemeSense - CS2 - 31 Days
    "5330": 12.9,  # MemeSense - CS2 - 90 Days
    "5340": 21.5,  # MemeSense - CS2 - 180 Days
    "5410": 1.29,  # Nixware - CS2 - 1 Day
    "5420": 3.0,  # Nixware - CS2 - 14 Days
    "5430": 4.75,  # Nixware - CS2 - 30 Days
    "5440": 9.5,  # Nixware - CS2 - 60 Days
    "5450": 12.5,  # Nixware - CS2 - 90 Days
    "5460": 2.15,  # Nixware - CS2 - HWID Reset
    "5510": 8.8,  # Kernaim - CS2 - 1 Week
    "5520": 14.7,  # Kernaim - CS2 - 1 Month
    "5530": 38.9,  # Kernaim - CS2 - 3 Months
    "5610": 17.65,  # Kernaim - Apex - 1 Week
    "5620": 41.0,  # Kernaim - Apex - 1 Month
    "5630": 81.5,  # Kernaim - Apex - 3 Months
    "5710": 19.99,  # Kernaim - ARC Raiders - 1 Week
    "5720": 39.99,  # Kernaim - ARC Raiders - 1 Month
    "5730": 79.99,  # Kernaim - ARC Raiders - 3 Months
    "5810": 19.99,  # Kernaim - Black Ops 7 - 1 Week
    "5820": 29.99,  # Kernaim - Black Ops 7 - 1 Month
    "5830": 69.99,  # Kernaim - Black Ops 7 - 3 Months
    "6110": 19.99,  # Kernaim - Rust - 1 Week
    "6120": 39.99,  # Kernaim - Rust - 1 Month
    "6130": 79.99,  # Kernaim - Rust - 3 Months
    "6210": 24.99,  # Kernaim - Escape from Tarkov - 1 Week
    "6220": 49.99,  # Kernaim - Escape from Tarkov - 1 Month
    "6230": 99.99,  # Kernaim - Escape from Tarkov - 3 Months
    "6310": 19.99,  # Kernaim - DayZ - 1 Week
    "6320": 39.99,  # Kernaim - DayZ - 1 Month
    "6330": 69.99,  # Kernaim - DayZ - 3 Months
    "6410": 19.99,  # Kernaim - MW3 - 1 Week
    "6420": 29.99,  # Kernaim - MW3 - 1 Month
    "6430": 69.99,  # Kernaim - MW3 - 3 Months
    "6510": 4.3,  # Fecurity – COD: BO7 / BO6 / WZ / MW3 / MW2 / MW19 - 1 Day
    "6520": 6.9,  # Fecurity – COD: BO7 / BO6 / WZ / MW3 / MW2 / MW19 - 3 Days
    "6530": 11.22,  # Fecurity – COD: BO7 / BO6 / WZ / MW3 / MW2 / MW19 - 1 Week
    "6540": 25.9,  # Fecurity – COD: BO7 / BO6 / WZ / MW3 / MW2 / MW19 - 1 Month
    "6550": 56.1,  # Fecurity – COD: BO7 / BO6 / WZ / MW3 / MW2 / MW19 - 3 Months
    "6610": 6.9,  # Fecurity – Battlefield 6 / 5 / 1 / 2042 - 1 Day
    "6620": 30.2,  # Fecurity – Battlefield 6 / 5 / 1 / 2042 - 1 Week
    "6630": 60.45,  # Fecurity – Battlefield 6 / 5 / 1 / 2042 - 1 Month
    "6710": 7.55,  # Fecurity – CS2 - 1 Day
    "6720": 12.95,  # Fecurity – CS2 - 14 Days
    "6730": 25.9,  # Fecurity – CS2 - 1 Month
    "6810": 4.3,  # Fecurity – Arc Raiders - 1 Day
    "6820": 25.75,  # Fecurity – Arc Raiders - 1 Week
    "6830": 55.8,  # Fecurity – Arc Raiders - 1 Month
    "6910": 13.99,  # Ethereal – GTA - Legacy, 1 Month
    "6920": 33.99,  # Ethereal – GTA - Legacy, Lifetime
    "6930": 14.99,  # Ethereal – GTA - Enhanced, 1 Month
    "6940": 39.99,  # Ethereal – GTA - Enhanced, Lifetime
    "7010": 10.0,  # Ethereal – RDR2 - 1 Month
    "7020": 25.0,  # Ethereal – RDR2 - Lifetime
    "7110": 28.9,  # Scooby – GTA - Premium Plus
    "7120": 11.5,  # Scooby – GTA - Premium Plus Upgrade
    "7130": 17.15,  # Scooby – GTA - Premium
    "7210": 8.5,  # Rift – FiveM - 1 Week
    "7220": 17.0,  # Rift – FiveM - 1 Month
    "7230": 34.1,  # Rift – FiveM - Lifetime
    "7301": 15.0,  # Jupiter – GTA - 7 Days
    "7302": 25.0,  # Jupiter – GTA - 30 Days
    "7303": 50.0,  # Jupiter – GTA - 90 Days
    "7304": 120.0,  # Jupiter – GTA - Lifetime
    "7310": 1.0,  # Jupiter – CS2 - 1 Day
    "7320": 2.0,  # Jupiter – CS2 - 1 Week
    "7330": 5.0,  # Jupiter – CS2 - 1 Month
    "7340": 20.0,  # Jupiter – CS2 - Lifetime
    "7410": 10.99,  # Susano – FiveM - 1 Week
    "7420": 16.99,  # Susano – FiveM - 1 Month
    "7430": 23.99,  # Susano – FiveM - 3 Month
    "7440": 29.99,  # Susano – FiveM - Lifetime
    "7510": 9.0,  # Scooby – RDR2 - Lifetime
    "7610": 3.99,  # TZ Project – FiveM PvP Tool - 1 Day
    "7620": 9.99,  # TZ Project – FiveM PvP Tool - 1 Week
    "7630": 19.99,  # TZ Project – FiveM PvP Tool - 1 Month
    "7640": 29.99,  # TZ Project – FiveM PvP Tool - Lifetime + Spoofer
    "7710": 4.99,  # TZX Project – FiveM External PvP Tool - 1 Day
    "7720": 19.99,  # TZX Project – FiveM External PvP Tool - 1 Month
    "7730": 34.99,  # TZX Project – FiveM External PvP Tool - Lifetime + Spoofer
    "7810": 4.99,  # Keyser – FiveM - 1 Day
    "7820": 8.99,  # Keyser – FiveM - 1 Week
    "7830": 12.59,  # Keyser – FiveM - 1 Month
    "7840": 31.49,  # Keyser – FiveM - Lifetime
    "7910": 15.2,  # LabCore – DayZ Pro - 1 Week
    "7920": 3.5,  # LabCore – DayZ Pro - 1 Day
    "7930": 30.4,  # LabCore – DayZ Pro - 1 Month
    "8010": 4.4,  # LabCore – DayZ DMA - 1 Day
    "8020": 13.0,  # LabCore – DayZ DMA - 1 Week
    "8030": 26.0,  # LabCore – DayZ DMA - 1 Month
    "8110": 2.6,  # LabCore – DayZ Radar - 1 Day
    "8120": 8.5,  # LabCore – DayZ Radar - 1 Week
    "8130": 17.0,  # LabCore – DayZ Radar - 1 Month
    "8140": 34.1,  # LabCore – DayZ Radar - 3 Month
    "8210": 4.35,  # Midnight – CS:GO - 30 Days
    "8310": 17.3,  # Scooby – FiveM - Lifetime
    "8401": 6.5,  # Rift – RDR2 - 7 days
    "8402": 17.5,  # Rift – RDR2 - 30 days
    "8403": 32.5,  # Rift – RDR2 - Lifetime
    "8420": 9.5,  # Nighty - Discord Selfbot - Lifetime
    "8510": 10.0,  # Reload – FiveM External - 1 Month
    "8520": 25.0,  # Reload – FiveM External - Lifetime
    "8610": 28.7,  # Scooby – RedM - Lifetime
    "8710": 5.0,  # Jupiter – Roblox - 30 days
    "8720": 10.0,  # Jupiter – Roblox - 90 days
    "8730": 20.0,  # Jupiter – Roblox - Lifetime
    "8810": 5.95,  # Nexoria – GTA - 1 Week
    "8820": 10.15,  # Nexoria – GTA - 1 Month
    "8830": 22.99,  # Nexoria – GTA - Lifetime
    "8840": 13.6,  # Nexoria – GTA - 1 Month BE
    "8910": 5.95,  # Yari – GTA - 7 Days
    "8920": 21.2,  # Yari – GTA - 30 Days
    "8930": 38.2,  # Yari – GTA - 60 Days
    "8940": 51.0,  # Yari – GTA - 90 Days
    "8950": 93.5,  # Yari – GTA - Lifetime
    "9010": 5.6,  # 0xCheat - RAGEMP - 7 Days
    "9020": 11.15,  # 0xCheat - RAGEMP - 30 Days
    "9110": 27.5,  # Cherax Prime – GTA - 1 Month
    "9120": 55.8,  # Cherax Prime – GTA - 3 Month
    "9130": 94.5,  # Cherax Prime – GTA - 6 Month
    "9210": 1.5,  # GSXR – GTA - 1 Day
    "9220": 5.0,  # GSXR – GTA - 1 Week
    "9230": 12.0,  # GSXR – GTA - Lifetime
    "9301": 4.4,  # Predator CS2 - 1 Month
    "9302": 12.0,  # Predator CS2 - 3 Months
    "9303": 48.0,  # Predator CS2 - 1 Year
    "9304": 1.5,  # Predator CS2 - 1 Day
    "9305": 3.5,  # Predator CS2 - 1 Week
    "9306": 24.0,  # Predator CS2 - 6 Months
    "9310": 4.3,  # Morphine - Rust External - 1 Day
    "9320": 21.5,  # Morphine - Rust External - 1 Week
    "9330": 43.0,  # Morphine - Rust External - 1 Month
    "9401": 7.53,  # Raiden - GTA - Regular
    "9402": 12.24,  # Raiden - GTA - Ultimate
    "9410": 3.45,  # Morphine - PUBG External - 1 Day
    "9420": 17.25,  # Morphine - PUBG External - 1 Week
    "9430": 34.5,  # Morphine - PUBG External - 1 Month
    "9510": 9.99,  # Noctyx - Valorant Skin Changer - 1 Day
    "9520": 19.99,  # Noctyx - Valorant Skin Changer - 1 Week
    "9530": 34.99,  # Noctyx - Valorant Skin Changer - 1 Month
    "9540": 49.99,  # Noctyx - Valorant Skin Changer - Lifetime
    "9610": 1.0,  # Pellix - CS2 - 1 Day
    "9620": 2.0,  # Pellix - CS2 - 7 Days
    "9630": 4.0,  # Pellix - CS2 - 30 Days
    "9710": 3.5,  # Atlas - Apex Legends - 1 Day
    "9720": 13.1,  # Atlas - Apex Legends - 7 Days
    "9730": 22.0,  # Atlas - Apex Legends - 30 Days
    "9810": 4.0,  # NewWay - GTA - Solo, 1 Week
    "9820": 10.0,  # NewWay - GTA - Solo, 1 Month
    "9830": 27.0,  # NewWay - GTA - Solo, Lifetime
    "9840": 10.0,  # NewWay - GTA - BE Bypass, 1 Week
    "9850": 30.0,  # NewWay - GTA - BE Bypass, 1 Month
    "9860": 55.0,  # NewWay - GTA - BE Bypass, 3 Month
    "9910": 4.0,  # NewWay - RDR2 - 1 Week
    "9920": 10.0,  # NewWay - RDR2 - 1 Month
    "9930": 27.0,  # NewWay - RDR2 - Lifetime
}
